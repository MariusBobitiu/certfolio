...
export default async function ProjectDetailPage({
  ...
  const publicProjectUrl =
    ...
    preferences?.publicProfile
      ? (`/u/${session.user.slug}/projects/${project.slug}` as Route)
      : null
  const publicProjectUnavailableReason =
    project.status !== "published"
      ? "Publish this project before sharing it publicly."
      : !session.user.slug
        ? "Add a public profile slug before sharing this project."
        : !preferences?.publicProfile
          ? "Turn on your public profile before sharing this project."
          : null
    ...
    <div className="relative space-y-8 overflow-hidden">
      ...
      <section className="rounded-4xl border border-border/70 bg-linear-to-br from-card via-card to-secondary/45 px-6 py-7 shadow-lg sm:px-8 sm:py-8 dark:border-white/8 dark:from-background dark:via-card/20 dark:to-card/30 dark:shadow-white/2">
        <div className="space-y-5">
          ...
          >
            <ChevronLeft className="mt-0.5 mr-1 h-4 w-4" />
            Back to projects
          </Link>
      ...
      <ProjectDetailForm
        ...
        publicProjectUnavailableReason={publicProjectUnavailableReason}
      />
    </div>
  )
}
